  ...
  return (
    <>
      ...
      <div className="pt-24 max-w-7xl mx-auto px-4 pb-20">
        <h1 className="text-3xl font-display font-bold mb-6">Movies</h1>
        ...
        {loading ? <LoadingGrid /> : <MediaGrid items={movies} type="movie" />}
        <div className="flex justify-center gap-2 mt-8">
          <button
            disabled={page <= 1}
            onClick={() => setPage((p) => p - 1)}
            className="px-4 py-2 bg-secondary rounded-lg text-sm disabled:opacity-30 hover:bg-surface-hover transition-colors"
          >
            Previous
          </button>
          ...
          <button
            onClick={() => setPage((p) => p + 1)}
            className="px-4 py-2 bg-secondary rounded-lg text-sm hover:bg-surface-hover transition-colors"
          >
            Next
          </button>
        </div>
